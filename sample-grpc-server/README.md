...
```lein new pedestal-service sample-grpc-server```

The output of the pedestal-service template was
augmented in the following ways:

1. Added resources/addressbook.proto
2. Modified the src/sample-grpc-server/service.clj to implement the GRPC service
...
2. Go to [localhost:8080](http://localhost:8080/) to see: `Hello from sample-grpc-server, backed by Protojure Template! `
...
If you then update the `require` block of sample-grpc-server/service.clj to use these generated clojure
